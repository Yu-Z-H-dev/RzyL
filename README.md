...
基于 NoneBot2 的 QQ 机器人，提供物理实验图片查询功能，支持精确匹配和 AI 模糊匹配，并附带彩蛋插件（图片彩蛋 + AI 模糊匹配 + 对话回退 + 随机一言）。
...
- **随机一言**：约 40% 概率在出图结果后追加一句随机文案，增加互动趣味
...
| `大雾2 ls` | 列出所有关键词 |
| `大雾2 help` | 帮助信息 |
| `彩蛋 <名称>` | 彩蛋查询，精确/AI 模糊匹配；均无匹配时作为对话机器人回复 |
| `彩蛋 ls` | 按类列出所有可用彩蛋 |
| `彩蛋 help` | 彩蛋帮助信息 |
...
彩蛋 DeepSeek         # 精确匹配别名，返回 AI 拟人类彩蛋图片（多图随机一张）
彩蛋 疯狂星期四       # 顶层个体彩蛋，多图随机输出一张
彩蛋 openai家的       # 不是任何别名 → 交给 AI 模糊匹配，命中则出图
彩蛋 今天好累啊       # 精确与 AI 均未命中 → 作为对话机器人引用回复你的发言
彩蛋 ls               # 按类列出所有可用彩蛋
彩蛋 help             # 查看彩蛋帮助
```

> 彩蛋的可查询范围由本地 `src/asserts/easter_egg/keywords.json` 中各级 `enabled` 开关决定，
> 上面只是举例；实际可用的名字以 `彩蛋 ls` 输出为准。`彩蛋 help` 的示例行会按当前可用彩蛋动态生成。
...
└── src/
    ...
    │       ├── factor/                 #     人物类资源（当前 enabled=false）
    │       ├── LLM/                    #     AI 拟人类资源
    │       ├── Origin/                 #     达菲朋友们类资源（当前 enabled=false）
    │       ├── cjx/                    #     顶层多照片个体（当前 enabled=false）
    │       ├── Nico/                   #     顶层多照片个体
    │       ├── CrazyThursday/          #     顶层多照片个体
    │       ├── Dory.jpg                #     顶层单照片个体
    │       └── ustc.jpg                #     顶层单照片个体
    ├── utils/                  # 公共工具
    │   ├── safe_send.py        #   发送容错（sendMsg 超时/失败不中断整个流程）
    │   └── ai_error.py         #   AI 错误码 → 可区分原因的用户提示
    └── plugins/
        ├── dawu/               # 主插件（大雾1）
        │   ├── __init__.py     #   命令处理
        │   ├── config.py       #   配置、群组过滤
        ...
        │   ├── keywords.py     #   注册表解析（enabled 过滤 + help 示例生成）
        │   ├── random_text.py  #   随机文本加载
        │   └── sentences.txt   #   随机一言
        └── echo/               # Echo 示例插件（代码全部被注释，实际未启用）
```
...
- **类（category）**：`factor`（人物）、`LLM`（AI 拟人）、`Origin`（达菲朋友们），每个类可整体开关（`enabled`）。当前仓库模板 `keywords.example.json` 中只有 `LLM` 为 `enabled: true`。
- **个体（entity）**：可被查询的最小编，分两种形态：
  - `mode: "file"` —— 单照片个体，输出唯一一张；
  - `mode: "dir"` —— 多照片个体（文件夹），随机输出一张。
- **顶层个体**：直接位于 `easter_egg/` 下、不属于任何类的个体，可单独设置 `enabled`。
- **复合体（compound）**：一个关键词映射到多个 target 个体（如 `four_sages` → 四位教师），其可见性受 target 所属类约束——target 所属的类被关闭时该复合体一并不可见（`factor` 关闭时「四大善人」即不可用）。
- **可见性规则**：只有「类」和「顶层个体」持有 `enabled` 开关；类内个体跟随所属类；不可见项不在 `彩蛋 ls` 中列出，也不会被精确匹配命中。
- **帮助示例**：`彩蛋 help` 的「例如」一行由 `keywords.py` 的 `build_help_examples()` 从当前可见彩蛋实时生成（每个开启的类取一个 → 顶层个体 → 可见复合体，最多 3 个），因此开关一改示例自动跟随，不会指向已禁用的名字；全部禁用时该行整体省略。
...
- 预设提示词从独立文件 `src/asserts/easter_egg/prompt_chat.md`（UTF-8，可任意换行排版；`prompt_chat.example.md` 仅为空占位文件）加载：文件缺失、无法读取或内容为空时**不附加任何提示词**，直接把用户在彩蛋后的原话交给 `MODEL_CHAT`（等价于直接向 AI 询问）；`MODEL_CHAT` 留空时对话回退不可用（会提示「AI 服务未配置，请联系管理员」）。
- AI 路径（模糊匹配 + 对话回退）按用户限流，每 60 秒最多 10 次请求。

### AI 请求实现与性能调优

三个 AI 插件（大雾1 / 大雾2 / 彩蛋）各自持有一份调用逻辑相同的 `ai_match.py`（大雾1 与大雾2 的两份逐字节相同；彩蛋那份另含对话回退），为避免推理模型请求超时做了如下处理：
